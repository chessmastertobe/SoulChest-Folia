...
public class DataManager {
    ...
    private void createTables() throws SQLException {
        try (Statement st = connection.createStatement()) {
            ...
            st.execute("""
                CREATE TABLE IF NOT EXISTS chest_items (
                    ...
                    FOREIGN KEY (chest_id)
                        REFERENCES soul_chests(id) ON DELETE CASCADE
                )
                """);
            ...
            st.execute("CREATE INDEX IF NOT EXISTS idx_chests_owner " +
                       "ON soul_chests(owner_uuid)");
        }
    }
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                try {
                    ...
                    plugin.getLogger().log(Level.WARNING,
                            "[SoulChest] Skipping corrupt chest row: " + e.getMessage());
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE,
                    "[SoulChest] Failed to load all chests", e);
        }
        return result;
    }
    ...
    public void saveChest(SoulChestData data) {
        ...
        String insertItem  =
            "INSERT INTO chest_items (chest_id, slot_type, slot_index, item_data) " +
            "VALUES (?,?,?,?)";
    ...
    public int getCustomLimit(UUID playerUUID) {
        try (PreparedStatement ps = connection.prepareStatement(
            ...
            plugin.getLogger().log(Level.WARNING,
                    "[SoulChest] Failed to read custom limit for " + playerUUID, e);
        }
        return -1;
    }
    ...
    public void setCustomLimit(UUID playerUUID, int limit) {
        if (limit <= -1) {
            try (PreparedStatement ps = connection.prepareStatement(
                ...
                plugin.getLogger().log(Level.WARNING,
                        "[SoulChest] Failed to remove custom limit for " + playerUUID, e);
            }
        } else {
            try (PreparedStatement ps = connection.prepareStatement(
                ...
                plugin.getLogger().log(Level.WARNING,
                        "[SoulChest] Failed to set custom limit for " + playerUUID, e);
            }
        }
    }
    ...
    private byte[] serializeItem(ItemStack item) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.WARNING,
                    "[SoulChest] Failed to serialize item " + item.getType(), e);
            return null;
        }
    }
    ...
    private ItemStack deserializeItem(byte[] bytes) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.WARNING,
                    "[SoulChest] Failed to deserialize item bytes", e);
            return null;
        }
    }
    ...
    private SoulChestData rowToData(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            plugin.getLogger().warning("[SoulChest] Invalid UUID in DB: " + uuidStr +
                    " (chest " + id + ") - skipping.");
            return null;
        }
        ...
        int     xp          = rs.getInt("stored_xp");

        List<ItemStack> contents = new ArrayList<>(Collections.nCopies(36, null));
        ItemStack[]     armour   = new ItemStack[4];
        ItemStack       offHand  = null;

        String itemSql = "SELECT slot_type, slot_index, item_data " +
                         "FROM chest_items WHERE chest_id = ?";
                    ...
                    switch (slotType) {
                        case "content" -> {
                            if (slotIndex >= 0 && slotIndex < 36)
                                contents.set(slotIndex, item);
                        }
                        case "armour"  -> {
                            if (slotIndex >= 0 && slotIndex < 4)
                                armour[slotIndex] = item;
                        }
